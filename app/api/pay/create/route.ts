  ...
  const approveLink = links.find(
    (link) =>
      (typeof link.rel === "string" &&
        link.rel.toLowerCase().includes("approve")) ||
...
function getAppBaseUrlFromHeaders(headers: Headers): string {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendPlanCode =
      requestedType || (normalizedPlanCode ? mapPlanCodeToBackend(normalizedPlanCode) : "");
    if (!backendPlanCode) {
    ...
    if (channel === "stripe") {
      ...
      const stripePayload = {
        project: requestedProject || "ambigramgenerator",
    ...
    const successUrl = `${appBaseUrl}/payment/success?channel=paypal`;
    const cancelUrl = `${appBaseUrl}/payment/cancel?channel=paypal`;
    const commonPayload = {
      project: requestedProject || "ambigramgenerator",
      type: backendPlanCode,
    ...
    const paypalIntent: PayPalIntent = body.paypalIntent || "subscription";
    const paypalPath =
      paypalIntent === "subscription"
        ? process.env.BILLING_PAYPAL_CREATE_SUBSCRIPTION_PATH ||
          "/prod-api/paypal/smart/create-subscription"
        : process.env.BILLING_PAYPAL_CREATE_ORDER_PATH ||
          "/prod-api/paypal/smart/create-order";
